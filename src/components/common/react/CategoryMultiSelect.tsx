 ...
 */
export default function CategoryMultiSelect({ locale, initialValues, placeholder, businessTypeName, onOpenChange }: CategoryMultiSelectProps) {
   ...
   */
  const mapCategoriesToOptions = (cats: ArticleCategory[], selectedValues: string[] = []) => {
  ...
  const normalizeBusinessTypeName = (value?: string) => {
    if (!value) return '';
    return value.trim();
  };
  const filterCategoriesByBusinessType = (cats: ArticleCategory[]) => {
    const targetBusinessType = normalizeBusinessTypeName(businessTypeName);
    if (!targetBusinessType) return cats;
    return cats.filter((item) => item.business_type_name === targetBusinessType);
  };
    ...
    fetchArticleCategories()
      .then((cats) => {
        ...
        if (Array.isArray(cats) && cats.length > 0) {
          ...
          const filteredCategories = filterCategoriesByBusinessType(cats);
          setSections([
            {
              id: 'category',
              title: t('common.category'),
              options: mapCategoriesToOptions(filteredCategories, selectedValues),
            },
          ]);
        } else {
          // Fallback defaults when API returns empty
          const selectedValues = normalizeSelectedValues(initialValues);
          const fallbackOptions = normalizeBusinessTypeName(businessTypeName)
            ? []
            : [
  ...
  return <MultiSelectBase locale={locale} sections={sections} onSectionsChange={setSections} placeholder={placeholder} changedEventName="category:changed" clearEventNames={['category:clear-all', 'filter:clear-all']} onOpenChange={onOpenChange} />;
}
