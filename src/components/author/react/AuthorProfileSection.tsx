      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-shrink-0">
            <Image 
              src={authorProfile.avatar_url || '/api/placeholder/80/80'} 
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-6 text-sm text-muted-foreground mb-3">
              <div className="flex items-center space-x-1">
                ...
                <span>{locale === 'us' ? 'followers' : '关注者'}</span>
              </div>
              <div className="flex items-center space-x-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span>{locale === 'us' ? 'articles' : '文章'}</span>
              </div>
            </div>

            {/* Bio */}
            {authorProfile.profile_bio && (
        ...
        <div className="flex items-center space-x-3 ml-4">
          ...
          <button 
            ...
            className="px-6 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center space-x-2"
          >
            {subscribing ? (
              <>
                ...
                <span>{locale === 'us' ? 'Subscribing...' : '订阅中...'}</span>
              </>
            ) : (
              <>
                <span>+</span>
                <span>{locale === 'us' ? 'Subscribe' : '订阅'}</span>
              </>
            )}
          </button>
        </div>
