...
7. **前端开发规范&注意事项**  
   ...
   - 必须避免单词拼写错误。  
   - 接口调用需要考虑防抖；耗时操作需对触发控件进行状态屏蔽，直到完成或失败后恢复。  
   - 需要结合用户特征与随机值生成唯一 msg id，并放在请求头或 cookie 中，避免重复提交。  
   - 用户文本提交需进行 XSS/CSRF 相关字符转义与过滤，不可完全依赖后端。  
   - 文本输入框需定义明确的正则校验与限制。  
   - 默认假设用户存在恶意提交意图，前端需做必要防护。
...
- **UI 设计规范与 Primitives（AI 辅助开发必读）**
  → `docs/UI_PRIMITIVES.md`
  → 包含色彩 token、字体规则、交互状态、布局系统、组件模式、自检清单等
